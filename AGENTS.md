...
- Начиная с plugin `1.4.8` live signed-runtime resolve может вернуть безопасный
  ...
  `Подразделение_Key` / `НаправлениеДеятельности_Key` →
  `ДоговорКонтрагента`, дедуплицирует UUID и публикует только фиксированные
  scalar-поля. Пользователь не может передать entity, URL, filter, select или
  orderby; OData string literal экранирует только runtime.
