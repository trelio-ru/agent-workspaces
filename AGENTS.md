...
- Для task-scoped работы смысловые изменения нужно предлагать оператору как
  один редактируемый комментарий с действием публикации, не останавливая Run.
  Неопубликованный вариант каждый раз пересобирается как краткая суть изменений
  после последнего реально опубликованного предложения, а не как сумма старых
  текстов. `create_comment` вызывается только после явной публикации
  оператором и не является условием handoff или submit.
...
- Начиная с plugin `1.4.8` live signed-runtime resolve может вернуть безопасный
  ...
  workflow-status остаётся `unknown` с причиной `register_not_published`, пока
  подтверждённый регистр состояния отсутствует в публикации.
