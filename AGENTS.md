...
- Начиная с plugin `1.5.0` постоянно подключённый bundled MCP host
  ...
  control plane не считается отсутствующей интеграцией. Тот же gate повторяет
...
- Начиная с plugin `1.4.8` live signed-runtime resolve может вернуть безопасный
  ...
  Runtime `1.0.11` для endpoint без validator сначала требует fixed HEAD с
  bounded total length, затем подтверждает `206` и exact `Content-Range`
  однобайтовым probe и только после этого параллельно читает оставшееся тело
  максимум тремя непересекающимися ranges. Суммарно читается ровно один
  metadata body без overlap; каждая часть обязана иметь тот же total и
  identity encoding. Неподдержанный HEAD/Range, изменившаяся длина или любая
  ошибка после probe fail-closed без второго полного GET.
