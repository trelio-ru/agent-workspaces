...
- Начиная с plugin `1.4.8` live signed-runtime resolve может вернуть безопасный
  ...
  workflow-status не выводится из косвенных признаков. Пока подтверждённый
  регистр состояния отсутствует в публикации, runtime opportunistic читает
  только фиксированные legacy-поля карточки
  `УдалитьСостояниеЭДО` / `УдалитьДатаИзмененияСостоянияЭДО`: непустое
  состояние возвращается с `basis=document_legacy_status_field` и
  `coverage=opportunistic`, пустое остаётся `unknown`, minimum-дата 1С
