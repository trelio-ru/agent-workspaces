...
- После принятия результата task-scoped Run агент может передать в
  `render_task_comment_proposal.filePaths` exact пути файлов из этого Run.
  Прикладываются только важные итоговые результаты и действительно полезные
  промежуточные файлы, нужные для понимания или продолжения работы, а не все
  файлы workspace, источники, технические и случайные outputs. Оператор видит
  точные карточки файлов, может убрать любую из них, а обычные task attachments
  создаются только при явной публикации предложения.
...
- Patch `1.6.10` добавляет в task-comment proposal осознанный выбор файлов из
  принятого task-scoped Run. Агент предлагает только важные итоговые результаты
  и действительно полезные промежуточные материалы; пользователь видит точные
  карточки, может убрать лишнее, а вложения создаются только при публикации
  предложения. Backend transport уже опубликован отдельно, поэтому новый
  plugin minimum, migration, env и OAuth scopes не требуются.
