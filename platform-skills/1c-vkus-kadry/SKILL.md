...
не удалённую и выполненную заявку
...
Runtime `1.2.1` исправляет source-specific признак готовности: заявка кабинета
остаётся `Posted=false` и после создания справки, поэтому generic-признак
проведения не является условием результата. Текущая команда выбирает девять
фиксированных полей, не возвращает email, комментарии или исполнителя,
fail-closed проверяет subject, удаление, `Выполнена`, дату и неоднозначность и
читает exact attachment source этой же заявки. Команда не создаёт заявку и не
извлекает число из PDF: агент скачивает один явно найденный файл существующей
справки и сообщает его собственную дату среза.
