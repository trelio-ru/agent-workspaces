...
<main id="app">
  <h1>Trelio — 1С ЭДО</h1>
  <p class="muted">Подготавливаю локальную форму…</p>
</main>
<script>
...
function renderWaiting() {
  ...
  app.innerHTML = `<h1>Trelio — 1С ЭДО</h1><p class="muted">Проверяю подключение…</p>`;
}
...
class BrowserPromptSession:
                ...
                with session.condition:
                    ...
                    if fields.get("cancel"):
                        ...
                        if len(value) > int(prompt["max_length"]):
                            self.send_json(
                                {"ok": False, "error": "Значение слишком длинное."},
                                status=400,
                            )
                            return
    ...
    def open(self) -> None:
        """Open the page and require the browser to fetch its exact URL."""
