...
После установки onboarding завершает OAuth, выбирает exact company и выполняет
первый обязательный read-only `get_agent_instructions`. Если текущие hooks уже
были одобрены, `PreToolUse` добавляет proof и настройка продолжается без паузы.
Для новой либо неодобренной definition protected content не возвращается; после
`TRELIO_RUNTIME_HOOK_REQUIRED` с `reason=missing` агент просит проверить Hooks.
В Codex Desktop пользователь открывает настройки `Trelio Agent Workspaces`,
просматривает текущую конфигурацию в разделе Hooks и включает её. В Codex CLI он
открывает `/hooks`, выбирает источник `Trelio Agent Workspaces`, проверяет
текущую конфигурацию и отмечает её доверенной. Codex не доверяет plugin-bundled
...
`codex mcp login trelio`; ручной переход в `Plugins` ради OAuth не является
основным fallback.
...
Обычная проверка новой версии идёт в отдельном процессе и переключает только
атомарный указатель для следующих запусков; уже работающий процесс и открытая
задача остаются на своём неизменяемом каталоге. Перед запуском долгоживущего
локального MCP loader отдельно выполняет bounded foreground-проверку и только
после неё открывает stdio transport: старый cache не должен закрепить процесс
ниже уже опубликованного minimum. При hard gate host runtime текущая
...
Неполный или повреждённый каталог exact `runtimeVersion/packageSha256` updater
под своей блокировкой удаляет и материализует заново; соседние версии и весь
runtime cache он не сканирует и не затрагивает.
...
Встроенные инструкции плагина и подробные правила навыков имеют один
канонический текст на русском. Поэтому агент может процитировать правило
в понятном виде. Команды, имена инструментов, поля API, коды ошибок и прежние
ссылки на разделы сохраняются. Перевод не меняет права, обязательные
подтверждения и условия восстановления.

По умолчанию агент отвечает по-русски, а при явном выборе пользователя – на
выбранном языке. Ограничение объясняет коротко: причина и следующий шаг.
Если нужны точная цитата и ссылка на правило, они сохраняются; перевод цитаты
на другой язык обозначается как перевод.
