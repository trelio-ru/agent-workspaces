...
После содержательного результата, включая частичную работу, отправленный ответ
или плановую проверку без Run, агент до proposals и финального ответа отдельно
оценивает текущий дедлайн и все видимые активные контроли по
[процедуре проверки дат](../plugins/trelio-agent-workspaces/skills/trelio-workspace-worker/references/task-date-review.md).
Выполненная проверка требует отдельного предложения о снятии, даже если задача
ещё в работе. Срок сопоставляется с оставшейся работой; обоснованный перенос
предлагается текстом и применяется только по exact решению либо действующему
поручению, разрешающему этот выбор. Автоматического переноса ради удаления
просрочки нет. Проверка дат не заменяет статус и чек-лист и не расширяет доступ.
