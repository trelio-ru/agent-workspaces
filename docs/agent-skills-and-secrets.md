...
`canReveal` сам проходит fresh auth и видит значение ограниченное время; при
отсутствии права создаётся обычный запрос `reveal`. Агент не управляет и не
инспектирует поверхность с открытым значением. Для `local_device` reveal нет.
