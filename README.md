...
При первом локальном `open` bridge покажет устройство и короткий pairing-код.
Агент покажет их пользователю и только после явного подтверждения одобрит код
через уже авторизованный MCP. Это не второй OAuth: MCP token не передаётся
bridge, а отдельная узкая device-session сохраняется локально и используется
во всех следующих Run без дополнительных MCP-запросов. Права на рабочие
правила, включая `mcp:agent-instructions:manage`, остаются только у основного
MCP-подключения.
