...
После этого полностью перезапустите Codex/ChatGPT и сначала повторите защищённое
чтение в этом же чате. Новый чат того же проекта нужен только если проверка здесь
снова не прошла. Диагностика использует тот же plan/apply flow и без
...
пустой папке только служебную Git-оболочку, local runtime классифицирует все
refs/objects и сохраняет допустимую `.git` на месте. Тот же host-side plan/apply
исключает `workspaces/`, проверяет index и снимки, обновляет активный `AGENTS.md`
или `AGENTS.override.md` и бережно добавляет его import в `CLAUDE.md`. Агент
только показывает exact preview и выполняет возвращённое CAS-bound действие;
Git и файлы shell-командами не собираются. Существующий или неоднозначный
репозиторий остаётся нетронутым. Блок требует проверять Trelio
...
Долгоживущий локальный MCP перед открытием transport синхронно сверяет current
signed runtime и не закрепляет уже несовместимую cached-версию. Открытая задача
продолжает использовать неизменяемый каталог, поэтому обычный runtime rollout
не требует новой задачи или перезапуска Codex. Если backend поднял минимальную
runtime-версию между двумя вызовами, loader дожидается подписанного обновления и
один раз сам перезапускает exact bridge-команду.
