...
без Git: связь с Trelio задаётся правилами компании и проекта, задачей, досье
...
кроме `AGENTS.md`/`AGENTS.override.md`, onboarding безопасно переименует `.git`
...
корневой `AGENTS.md`, подключит локальный компонент без тестового рабочего
запуска и предложит настроить нужные навыки. Для
...
Если нужно только прочитать или проверить уже сохранённые материалы задачи или
досье, агент вызывает `prepare_agent_workspace_read`, затем выполняет точную
