...
одной bridge-командой `finish`: она проверяет delta, сохраняет понятный handoff
и отправляет candidate. Если рабочая область успела измениться после старта,
Trelio отклоняет запись с `WORKSPACE_OUTDATED`, чтобы изменения были перенесены
в новую версию осознанно. После принятого task Run системный комментарий
остаётся техническим контекстом для агентов, а отдельный human-facing proposal
