...
description: Поиск и загрузка актуальных Agent Skills и опубликованных Agent Procedures компаний и проектов Trelio через MCP. Используй после авторизации Trelio, когда повторяемая процедура может определять работу, по запросу доступных навыков и до подключения или использования интеграции, которую может предоставить Trelio. Для общего запроса интеграции сначала проверь каталог Trelio, затем решай об установке, авторизации или вызове аналогичной сторонней интеграции.
---
...
Agent Skills Trelio – актуальные дополняющие инструкции компании/проекта. Они
сосуществуют с личными навыками пользователя. Agent Procedure – опубликованная
project-scoped Markdown-инструкция для повторяемой работы, но не scheduler или
runtime. Отсутствие назначения skill означает лишь отсутствие этого навыка
Trelio в контексте, а не запрет компании.
...
2. Когда для задачи правдоподобна reusable procedure или подключённый сервис,
   вызови `search_agent_guidance` один раз с точной областью,
   ...
   Ответ проекта уже объединяет опубликованные процедуры и эффективные
   назначения skills, указывает `kind` и exact read tool каждого результата.
3. Выбирай по компактной выдаче, рангу и признакам совпадения, не загружая
   инструкции всех результатов заранее. `kind=procedure` читай через exact
   `get_agent_procedure` и исполняй только published revision; draft/comments
   не являются инструкциями, а сама процедура не запускает фоновую работу.
   Её required Agent Skills загружай через `get_agent_skill`, а Agent Secrets
   используй только по exact ID/binding через protected flow без значения в
   prompt. При onboarding сначала выполни
...
Если `search_agent_guidance` не нашёл подходящего назначенного навыка, совместимый
...
Если недоступны сами `search_agent_guidance`/`get_agent_skill`/`get_agent_procedure`, сообщи о
...
Настройка любого навыка (Markdown, Remote MCP или runtime), личный вход,
переподключение, установка зависимостей и проверка готовности выполняются без
задачи, Workspace и Agent Run. Не проси ссылку на задачу и не создавай Run ради
настройки либо сохранения её технического итога. Company/project scope нужен
для назначения и ACL, но не означает task/Run scope. Если безопасного setup-пути
нет, назови конкретную блокировку навыка; checkout с фиктивным Run её не исправляет.
...
- команды без company secret запускай по инструкции навыка без secret checkout;
  `prepare_agent_secret_checkout` и exact executable нужны только рабочим
  операциям с company secret, для которых текущий контракт требует Run;
