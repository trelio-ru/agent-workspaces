...
description: Work through Trelio company, project, or task Agent Workspaces with MCP and the local Git bridge. Use when the user asks Codex to take, continue, analyze, prepare materials for, or complete work tied to a Trelio company/project/task, and the durable result must be checkpointed and submitted for review.
---
...
11. Before submission run `trelio-workspace status` and inspect every changed path. Then create a `handoff` checkpoint with a plain-language result summary, one or more result/validation items, the materials prepared for review, every open question, and one concrete operator action. For task work, pass the `comment.id` returned by `create_comment` as `--task-comment`. For example: `trelio-workspace checkpoint --type handoff --summary "Подготовлен план монтажа с ответственными и контрольными точками." --evidence "Исходные требования сопоставлены с планом; критических технических препятствий не обнаружено." --file artifacts/montage-plan.md --question "Кто подтверждает дату монтажа?" --task-comment <comment-uuid> --next-action "Проверьте план, ответьте на открытый вопрос и примите либо верните результат."`.
12. Run `trelio-workspace submit`. The bridge commits changes, heartbeats the lease, creates the candidate bundle, and sends it for review. Trelio rejects submission without a meaningful handoff and, for task work, without the linked operator-facing task comment.
13. Report to the operator in this order: outcome, important findings or validations, materials ready for review, open questions, and the exact action needed from them. Keep identifiers and implementation details out of the normal response; mention a short revision only during troubleshooting. Never say merely that useful content is "inside" the candidate—surface the content or name the exact material. Do not accept your own result; acceptance is a separate human action in Trelio.
...
- On `LEASE_EXPIRED` or stale fencing, do not retry mutations with old identifiers. If continuing your own existing Run is intentional, claim it again through `trelio-workspace open --workspace <uuid> --run <uuid>`; otherwise start a new Run from the current accepted head and reapply only reviewed local changes.
- On `WORKSPACE_OUTDATED`, preserve the rejected candidate. Start a new Run, compare it with the new accepted head, and merge deliberately; never force-update the canonical revision.
