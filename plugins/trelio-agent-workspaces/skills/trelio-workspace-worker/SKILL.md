...
обеспечивает локальный Git. Для каждой цели соблюдай ACL компании,
...
Каждое дополнение пользователя классифицируй отдельно.
Полностью прочитай все подходящие references ниже до первого связанного вызова инструмента.
...
после compaction. Не читай несвязанные;
нужный reference используй повторно, пока его версия и полный текст остаются в
контексте, иначе перечитай. Свежие `proposalContexts` заменяют
