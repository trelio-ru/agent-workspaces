...
description: >-
  ...
  проверки чек-листа или предложения его состояния, контролей, правил и
...
внешним сервисом или Run не поглощает позднюю отдельную просьбу. Подходящие
references дополняют друг друга; не читай несвязанные заранее. Повторно используй
SKILL/reference, пока точная версия и полный текст остаются в контексте, включая
следующие ходы. После compaction, потери текста, смены версии или появления
нового сценария перечитай. Подходящие свежие `proposalContexts` переиспользуй
для предусмотренного reference чтения контекста.
...
- **Чтение/ревью принятого Workspace без изменений:**
  ...
  чтения; при поздней просьбе о записи прочитай Run references.
...
- **Редактируемое предложение комментария или ответа, включая обязательное
  ...
  Поздняя просьба о proposal остаётся отдельным native-маршрутом Trelio даже
  при работе над исходниками или после compaction. Если точная задача
  неизвестна, прочитай область/контекст.
- **Открытие task Run, прямая просьба о статусе, вывод о готовности или итоговая
  ...
  Всегда читай до открытия task Run. В конце каждого содержательного task Run
  оцени всю задачу до необязательного вопроса или финального ответа, отдельно
  от обязательного комментария человеку.
- **Просьба изменить чек-лист, вывод о прогрессе пунктов или принятый task Run:**
  ...
  После принятия оцени каждый пункт, даже если вся задача не готова; решения
  о комментарии, статусе и контролях независимы.
...
- **Task-scoped Run с записью:**
  ...
  submit и итогового отчёта. Reference статусов определяет однократное решение
  о начале и позднее решение о завершении; reference чек-листа – повторную
  оценку пунктов после принятия.
