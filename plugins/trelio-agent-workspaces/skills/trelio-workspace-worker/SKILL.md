...
9. Run relevant validation. After bridge `open`, prefer `trelio-workspace checkpoint`: the bridge has claimed the Run and persists the current lease/fencing pair locally. Include result summary, evidence, changed files, open questions, and next action; never include private chain-of-thought.
10. Before submission run `trelio-workspace status`, inspect every changed path, then run `trelio-workspace submit`. The bridge commits changes, heartbeats the lease, creates the candidate bundle, and sends it for review.
11. Report the candidate revision and review status. Do not accept your own result; acceptance is a separate human action in Trelio.
