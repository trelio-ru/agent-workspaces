...
1. До пяти независимых запросов; в каждом сохраняй известный предмет-якорь,
   меняя синоним, старое имя, объект или результат.
2. Один раз вызови единый `search` с отдельными `queries` и всеми точными
   ...
   между компаниями. Не склеивай формулировки. Первый вызов – `limit=5`;
   повышай лишь при неоднозначности всей пятёрки.
