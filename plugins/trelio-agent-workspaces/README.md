...
Git-оболочку хоста с инструкциями и `.gitignore` onboarding сохраняет на месте.
Агент сам исключает весь `workspaces/` из Git корневой папки и проверяет, что
рабочие данные отсутствуют в её index и снимках. Ручное удаление, переименование
или изменение прав `.git` не требуются. Существующий или неоднозначный репозиторий не меняет и
