...
- регулярные работы с date-only расписанием компании, lightweight-проверками и
  шаблонами обычных задач;
...
Pinned-каталоги `context/company`, `context/project` и `context/related/*`
точно соответствуют текущему Run: после успешной синхронизации bridge удаляет
dependency-каталоги, которых больше нет в новом `contextHeadsJson`.
...
accepted-история при этом сохраняется. `clean --dry-run` объясняет, почему
остальные roots сохранены. Истёкший sibling Run не считается открытым, но root
самого `expired` Run не удаляется; небольшой обычный `.DS_Store`, `Thumbs.db`
или `desktop.ini` не блокирует cleanup. Автоматическая проверка выполняется
после открытия, успешного завершения или локальной отмены Run, не чаще одного
успешного прохода в сутки для одного Trelio origin.
...
После установки onboarding сначала завершает OAuth, выбирает exact company и
сам выполняет обязательный read-only `get_agent_instructions`. Если текущие
hooks уже были одобрены, `PreToolUse` добавляет proof и настройка продолжается
без паузы. Для новой либо неодобренной definition backend fail-closed не
возвращает protected content; после `TRELIO_RUNTIME_HOOK_REQUIRED` с
`reason=missing` агент попросит проверить Hooks. В Codex Desktop откройте
...
доверенной. Codex не доверяет plugin-bundled hooks автоматически и пропускает
...
после записи требуется полный restart Codex/ChatGPT. Затем защищённое чтение
сначала повторяется в этом же чате; новый чат того же проекта нужен только если
проверка здесь снова не прошла. Диагностика использует тот же plan/apply flow и
остаётся read-only без отдельного согласия.
...
обычно без Git: Trelio-контекст приходит из правил компании и проекта, задачи, воркспейса
или их Agent Workspace, а не из связи с репозиторием. Проверенную служебную
Git-оболочку хоста onboarding сохраняет через host-side read-only plan и
CAS-bound apply. Runtime сам проверяет refs/objects/index, исключает
`workspaces/`, обновляет активный `AGENTS.md` или `AGENTS.override.md` и добавляет
его import в `CLAUDE.md`, сохраняя посторонние инструкции. Агент показывает
preview и выполняет exact action без shell-записи этих файлов. Ручное удаление,
переименование или изменение прав `.git` не требуется; существующий или
неоднозначный репозиторий остаётся нетронутым. Блок требует проверять Trelio до
...
fallback в плагине отсутствует. Последующие обновления обычно проверяются в
отдельном процессе и переключают только будущие запуски; перед открытием
долгоживущего локального MCP loader выполняет bounded foreground-проверку, чтобы
не закрепить уже несовместимый cached runtime. Текущий процесс не удаляется и
не заменяется. Поэтому compatible
...
Если проверенный каталог сохранился, но `current.json` потерян или повреждён,
...
mirror: задачи, воркспейсы, база знаний, контакты, реестры, встречи и регулярные
работы попадают туда
