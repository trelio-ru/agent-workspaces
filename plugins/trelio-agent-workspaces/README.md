...
покажет устройство и короткий pairing-код. Агент должен показать их
пользователю и только после явного подтверждения одобрить exact код через уже
авторизованный MCP. Bridge докажет владение локальным verifier и сохранит
узкую device-session в macOS Keychain или приватном файле. Второй OAuth не
запускается, MCP token не копируется, а следующие Run используют ту же
device-session без дополнительных MCP-запросов. Права на рабочие правила,
включая `mcp:agent-instructions:manage`, остаются у основного MCP.
