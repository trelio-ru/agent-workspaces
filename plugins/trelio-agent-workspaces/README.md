...
`Trelio` и завершите OAuth-доступ в `Trelio Agent Workspaces`. Первая команда
подключает зафиксированную стабильную версию marketplace, вторая устанавливает
из него плагин. После OAuth полностью перезапустите Codex и начните новую задачу,
чтобы сессия перечитала MCP-инструменты. При первом локальном открытии workspace
bridge отдельно запустит OAuth PKCE и сохранит credential в macOS Keychain.
...
лично. Ветка `main` уже использует `INSTALLED_BY_DEFAULT`; это уберёт вторую
команду после публикации следующего plugin-релиза. В текущей стабильной
`v1.2.3` явная установка остаётся обязательной.
