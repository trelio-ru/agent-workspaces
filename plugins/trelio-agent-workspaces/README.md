...
Для каждого task/dossier Workspace bridge использует одну постоянную папку
`~/Trelio Workspaces/<workspace-id>/workspace`. Следующие Agent Run работают в
ней же: перед открытием bridge сверяет live `acceptedHead` и статус предыдущего
Run с Trelio, проверяет чистоту локального Git и только затем синхронизирует
tracked-файлы. Локальные изменения никогда не перезаписываются автоматически,
а без доступного server state новый writable Run не открывается. В одной папке
одновременно работает один локальный Run.

Служебные `.trelio-run.json` и `context/` находятся уровнем выше рабочей папки;
выходные файлы агента остаются в видимом `workspace/`. Чистая локальная копия
...
route ведёт к двум компактным local tools; агент не решает сам, какой provider
выбрать, а local host повторно проверяет live state.
...
без Git: Trelio-контекст приходит из правил компании и проекта, задачи, досье
...
без тестового Run и предложит настроить только выбранные навыки. Для exact
...
Для чистого чтения task/dossier Workspace агент вызывает
...
mirror: задачи, досье, база знаний, контакты, реестры и встречи попадают туда
