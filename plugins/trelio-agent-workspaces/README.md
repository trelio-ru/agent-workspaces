...
результата отправки. Команда `probe` возвращает только безопасный UI-fingerprint
без текстов чатов, сообщений, cookies и credentials. Если интерфейс нельзя
распознать однозначно, runtime закрывается с ошибкой и не отправляет сообщение.
