...
Через MCP агент может прочитать эффективные правила, подготовить точный diff и
опубликовать новую версию только отдельным вызовом после явного подтверждения
пользователя. Для публикации нужен scope
`mcp:agent-instructions:manage`; старому OAuth-подключению требуется повторный
consent.
