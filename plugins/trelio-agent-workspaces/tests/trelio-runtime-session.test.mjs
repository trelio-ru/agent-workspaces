...
test("an active hook preserves the plugin upgrade code instead of claiming Hooks are disabled", async () => {
  ...
  const server = createServer((request, response) => {
    ...
    assert.equal(request.headers["x-trelio-agent-workspaces-version"], "1.14.1");
    ...
    if (request.url === "/api/agent-workspaces/bridge-compatibility") {
      ...
      response.end(JSON.stringify({ supported: false, minimumVersion: "1.14.2" }));
      return;
    }
  ...
  try {
    ...
    assert.match(result.stderr, /v1\.14\.1 больше не поддерживается; требуется v1\.14\.2/u);
