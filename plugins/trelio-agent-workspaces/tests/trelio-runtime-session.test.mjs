...
test("an active hook preserves the plugin upgrade code instead of claiming Hooks are disabled", async () => {
  ...
  const server = createServer((request, response) => {
    ...
    assert.equal(request.headers["x-trelio-agent-workspaces-version"], "1.16.6");
    ...
    if (request.url === "/api/agent-workspaces/bridge-compatibility") {
      ...
      response.end(JSON.stringify({ supported: false, minimumVersion: "1.16.7" }));
      return;
    }
  ...
  try {
    ...
    assert.match(result.stderr, /v1\.16\.6 больше не поддерживается; требуется v1\.16\.7/u);
