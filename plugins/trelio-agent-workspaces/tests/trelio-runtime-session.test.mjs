...
test("an active hook preserves the plugin upgrade code instead of claiming Hooks are disabled", async () => {
  ...
  const server = createServer((request, response) => {
    ...
    assert.equal(request.headers["x-trelio-agent-workspaces-version"], "1.17.12");
  ...
  try {
    ...
    assert.match(result.stderr, /v1\.17\.12 больше не поддерживается; требуется v1\.17\.13/u);
