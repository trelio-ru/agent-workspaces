...
test("an active hook preserves the plugin upgrade code instead of claiming Hooks are disabled", async () => {
  ...
  const server = createServer((request, response) => {
    ...
    assert.equal(request.headers["x-trelio-agent-workspaces-version"], "1.17.10");
    ...
    if (request.url === "/api/agent-workspaces/bridge-compatibility") {
      ...
      response.end(JSON.stringify({ supported: false, minimumVersion: "1.17.11" }));
      return;
    }
  ...
  try {
    ...
    assert.match(result.stderr, /v1\.17\.10 больше не поддерживается; требуется v1\.17\.11/u);
