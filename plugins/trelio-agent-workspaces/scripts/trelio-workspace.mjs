...
export const BRIDGE_VERSION = "1.6.10";
...
export const AGENT_WORKSPACE_RUNTIME_AGENTS_MARKDOWN = [
  ...
  "- Если в работе с задачей появились смысловые изменения, прочитай `get_task_comment_proposal_context` и вызови `render_task_comment_proposal` с новой краткой смысловой сводкой после последнего реально опубликованного предложения. После принятия результата task-scoped Run передай в `filePaths` только важные итоговые результаты и действительно полезные промежуточные файлы, нужные для понимания или продолжения работы; не прикладывай все workspace-файлы, источники, технические или случайные outputs по умолчанию. MCP App даёт человеку редактируемое поле, точные карточки выбранных файлов и кнопку «Опубликовать»; человек может убрать любой файл, а обычные task attachments создаются только при публикации. Не публикуй автоматически и не останавливай из-за неопубликованного текста работу. Новый render заменяет прежний неопубликованный вариант, а не дополняет его копиями старого текста. В клиенте без MCP Apps покажи fallback-текст и предложенные файлы, а `publish_task_comment_proposal` вызывай только после явной команды пользователя. Для этого proposal не используй `create_comment`. Handoff и submit от manual comment не зависят.",
