...
export const buildAgentWorkspaceRuntimeAgentsMarkdown = (
  ...
  "- Если пользователь прямо просит показать значение Agent Secret, направь его в защищённый browser reveal Trelio exact карточки: проверь safe `canReveal`, при отсутствии запроси `reveal` access. Пользователь сам проходит fresh auth и видит временное значение локально; не возвращай plaintext в чат и не открывай/не инспектируй reveal через Browser, Chrome или Computer Use. Для `local_device` browser reveal отсутствует.",
  "",
  "## Работа и результат",
  "",
