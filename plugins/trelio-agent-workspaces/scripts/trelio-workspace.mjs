...
export const buildAgentWorkspaceRuntimeAgentsMarkdown = (
  ...
  "- Статус — отдельное решение по всей задаче: accepted Run и `taskOutcome` его не меняют, а выполненное поручение может быть лишь частью задачи. Перед финальным ответом после каждого содержательного task Run обязательно заново оцени всю задачу и не жди отдельного вопроса пользователя о статусе. Отсутствующий необязательный срок, исполнитель, контроль или будущая профилактика сами по себе не являются unresolved question и не переводят готовую задачу в `no_status_change`; блокируют только требования самой задачи или target transition policy. Для partial work используй `no_status_change`, готовь только обязательный comment proposal и не создавай status proposal. Если вся задача готова, даже при уже записанном `no_status_change`, отдельно прочитай `get_task_status_proposal_context` и вызови `render_task_status_proposal`; `apply_task_status_proposal` / `dismiss_task_status_proposal` разрешены только после действия пользователя в MCP App или явного решения по exact proposal.",
