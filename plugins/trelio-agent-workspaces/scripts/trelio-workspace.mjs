...
export const BRIDGE_VERSION = "1.10.10";
...
export const buildAgentWorkspaceRuntimeAgentsMarkdown = (
  ...
  "- Политику модели применяет approved hook плагина: discovery и recovery доступны без допуска, а в context, mutation и Agent Workspace hook сам подставляет одноразовый runtimeSessionProof. Никогда не создавай и не копируй proof или model attestation вручную. При TRELIO_RUNTIME_HOOK_REQUIRED останови защищённую работу и в Codex скажи пользователю только: «Откройте настройки плагина Trelio Agent Workspaces, включите Hooks и повторите запрос». В Claude Code/Cowork попроси только enable/approve hooks плагина и повторить запрос. Не предлагай заранее установку, обновление или переустановку плагина, `trelio-workspace login`, новую задачу/session или restart; переходи к ним только после включения Hooks, если повтор доказал, что текущая session их не подхватила, либо вернул отдельную конкретную ошибку version/install/pairing/session. Не обходи gate другим MCP, HTTP, browser или shell.",
  "",
  "## Начало Run",
  "",
  ...
  "- Вызови `propose_task_comment` один раз: для direct proposal передай exact task locator; после содержательного accepted task Run передай `runId` и короткий человеческий итог, а предложение укомплектуй только полезными `filePaths`. System handoff — технический аудит и контекст для агентов, а обычный proposal — коммуникация для людей. Не публикуй автоматически и не используй `create_comment`; без MCP Apps вызывай `publish_task_comment_proposal` или `dismiss_task_comment_proposal` только по явной команде. Для сложной коррекции или нового mention сначала прочитай `get_task_comment_proposal_context` и вызови `render_task_comment_proposal` с exact snapshot hash. Перед финальным ответом proposal должен вернуть draft либо точный blocker.",
