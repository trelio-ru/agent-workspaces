...
export const BRIDGE_VERSION = "1.6.13";
...
export const AGENT_WORKSPACE_RUNTIME_AGENTS_MARKDOWN = [
  ...
  "- Используй только exact `runtimeExecution` command или объявленный `remoteMcpExecution` host/identity/release. Начальный `trelio-workspace` — логический launcher текущего плагина: используй PATH либо bundled bridge этой версии через Node.js 22+, не сканируй cache, не сообщай о штатно отсутствующем PATH и не запускай пробный failure. Не обходи навык браузером, Computer Use, прямым HTTP, альтернативным MCP или скриптом.",
  "- Fallback допустим только когда релевантного навыка нет, обязательное подключение не настроено или операция не поддерживается; назови причину. Недоступность каталога — сбой control plane, не отсутствие интеграции. На `AGENT_SKILL_RELEASE_CHANGED` перечитай навык. Совместимые личные навыки не запрещены.",
