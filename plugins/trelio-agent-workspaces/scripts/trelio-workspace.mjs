...
export const BRIDGE_VERSION = "1.13.9";
...
export const buildAgentWorkspaceRuntimeAgentsMarkdown = (
  ...
  "- Trelio MCP и bundled bridge остаются штатным workflow поиска задач, Workspace/Run, context, checkpoint, submit и restore; не ищи для этих операций отдельный catalog skill. Для discovery после явных task/dossier связей используй один канонический ACL-aware `search` с несколькими запросами и exact company scope: он вместе возвращает проекты, задачи и accepted task/dossier Workspace files. `search_tasks` и `search_agent_workspace_files` используй только для узкого уточнения, не как обязательную последовательность; не вызывай list_dossiers только ради discovery. При structured `MCP_SEARCH_TIMEOUT` не делай generic network retry и не повторяй широкий запрос: максимум один раз передай exact `companySlugs`, не больше двух сильнейших независимых queries и exact `projectSlugs` только для уже известной task-only границы; если честно сузить scope нельзя либо retry вернул тот же code, остановись и запроси недостающий discriminator. Не склеивай формулировки, не переходи к browser/HTTP/другому MCP или цепочке одиночных запросов. Bare HTTP 504 без structured code остаётся transport failure. Для одного exact task используй `get_task`, для 2-20 exact targets — один `get_tasks`, не повторяй `get_task`. В task-read schema v2 читай полный ответ из `structuredContent`, для каждого `tasks[]` разрешай только его `instructionScope.orderedLayerKeys` через `effectiveInstructions.layers` и применяй exact порядок; не переноси company/project/personal layer на непривязанную задачу и не дублируй весь catalog для каждой. Company/project rules не являются поисковыми документами. Вне Run обычные exact `fetch`/`get_dossier` возвращают envelope, а task reads — layered refs; внутри Run используй pinned instruction/profile snapshot и не заменяй его более новой exact-read revision.",
  ...
  "- Статус — отдельное решение: открытие и accepted Run сами его не меняют. Сразу после успешного `trelio-workspace open` для task Run ровно один раз прочитай `get_task_status_proposal_context`; только при `workStartProposal.state=eligible` покажи `render_task_status_proposal` с `intent=work_started`, exact queue/active target и продолжай работу без ожидания. Не повторяй уже показанное start-предложение после действий, checkpoint, pause, нового хода, pending draft, отклонения или замены completion-карточкой в той же status epoch и не обходи suppression task locator-ом. Перед финальным ответом после каждого содержательного task Run обязательно заново оцени всю задачу и не жди отдельного вопроса пользователя о статусе. Отсутствующий необязательный срок, исполнитель, контроль или будущая профилактика сами по себе не являются unresolved question и не переводят готовую задачу в `no_status_change`; блокируют только требования самой задачи или target transition policy. Для partial work используй `no_status_change`, готовь только обязательный comment proposal и не создавай completion proposal. Если вся задача готова, даже при уже записанном `no_status_change`, отдельно прочитай live context и вызови status proposal с `intent=whole_task_ready`; он заменяет pending `work_started`. `apply_task_status_proposal` / `dismiss_task_status_proposal` разрешены только после действия пользователя в MCP App или явного решения по exact proposal.",
  ...
  "- Состояние чек-листа — отдельное решение. После каждого содержательного accepted task Run перечитай live checklist и предложи только прямо подтверждённые результатом переходы ordinary non-linked items; partial work может предложить exact выполненные пункты без готовности всей задачи. Не предлагай текст, добавление, удаление, порядок или linked-subtask state и не пиши ритуальное «чек-лист не менял». Для единственной карточки используй `get_task_checklist_proposal_context` и `render_task_checklist_proposal`, а вместе с обязательным comment либо другим proposal — один `render_task_proposals` с `checklistProposal`. Пользователь может снять выбор с отдельных пунктов; `apply_task_checklist_proposal` / `dismiss_task_checklist_proposal` разрешены только после его действия в App или явного решения по exact proposal. Private reasons не публикуй.",
  "- Immediate `complete_checklist_item` и изменение `isCompleted` существующего пункта через `update_checklist` используй только после прямой однозначной команды человека изменить exact пункт сейчас и только с `userExplicitlyRequestedImmediateChecklistStateChange=true`. Accepted Run, inferred progress или общая просьба актуализировать задачу этого права не дают; не обходи proposal созданием нового completed item или пересозданием чек-листа.",
  "- Task payload из `get_task` / `get_tasks` показывает shared и только твои personal controls. Это date-only проверки, не дедлайны: дата не уведомляет. Используй `create_task_control` / `update_task_control` / `clear_task_control` только при конкретной необходимости; не расширяй personal в shared без полномочия и не снимай контроль только из-за завершения Run или смены статуса. Shared changes видны в task audit, personal остаются приватными.",
  "- Заверши Run одной командой `trelio-workspace finish` с результатом, подтверждениями, материалами, вопросами и одним следующим шагом. Она сама проверяет полный candidate delta, включая уже сохранённый draft checkpoint, создаёт handoff, продлевает lease и отправляет candidate; не делай искусственную финальную правку только ради dirty status. Для task scope оцени всю задачу и выбери semantic `--task-outcome` из options подготовленного Run: `work_completed`, `review_passed`, `direct_completion` или safe default `no_status_change`; только при completion-blocking вопросах и partial work используй `no_status_change`, а пустые необязательные task metadata не превращай в `--question`. Outcome — recommendation для отдельного status proposal, не task mutation.",
