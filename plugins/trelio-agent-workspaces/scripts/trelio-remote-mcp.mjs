...
export const AGENT_SKILL_ROUTING_INSTRUCTIONS = [
  "Native Trelio task/Workspace/Run не требуют поиска навыка. Следуй только серверному providerSelection; не выводи локальный маршрут сам.",
  ...
  "Перед финалом содержательной работы и внешнего поиска без Run выполни trelio-workspace-worker/references/workspace-context-review.md по effective rules.",
  "До установки, авторизации или вызова другой внешней интеграции определи компанию Trelio; при нескольких спроси. Используй search_agent_skills, list_agent_skills – только для явного inventory. До первого внешнего действия загрузи точный включённый get_agent_skill. Полный текст переиспользуется между ходами до 12 часов при неизменных session/context/skill/implementation/intent. Перечитай при новой сессии, потере/compaction текста, expiry, смене маршрута, снятии blocker или AGENT_SKILL_RELEASE_CHANGED. Не продлевай допуск host. Отсутствие активного tool не доказывает отсутствие навыка.",
