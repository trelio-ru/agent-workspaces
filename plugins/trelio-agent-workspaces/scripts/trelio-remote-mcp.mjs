...
export const AGENT_SKILL_ROUTING_INSTRUCTIONS = [
  "Trelio Agent Skill routing gate: When a user asks to connect or use an external integration that Trelio may provide, resolve the intended Trelio company before installing, authorizing, or invoking an overlapping native/plugin integration. If the request does not identify a company and several are available, ask which Trelio company applies instead of scanning unrelated catalogs or silently choosing the non-Trelio integration. In the resolved company or project context, call `list_agent_skills` for that exact company and project. Select a relevant assigned skill by its purpose, then immediately before the action call `get_agent_skill` for the same context. Do this even when no integration-specific tool appears in the active tool list; a missing active tool is not evidence that the integration is unavailable.",
  "Telegram routing is deterministic from the catalog's formal `integrationRouting`, never array order or titles. If exactly one of `telegram-mtproto` or `telegram-web` is enabled, use that assigned skill. If both are enabled, use `telegram-mtproto` as primary priority `100`; use `telegram-web` as secondary priority `200` only after the primary has exactly established `not_configured`, `no_access`, `needs_reconnect`, or `unsupported_operation`. Catalog/control-plane unavailability, timeout, transient or unknown failure, and an ambiguous mutation outcome never permit transport fallback or an automatic repeat. Establish the live mutation result first or ask the user whether to retry.",
  "Use only an exact `runtimeExecution` command or the declared `trelio-remote-skills` tools with returned identity/release. A leading `trelio-workspace` is the logical launcher of this plugin: use PATH or this version's bundled bridge through Node.js 22+, without scanning caches, announcing a normally absent PATH entry, or running a failing probe. Never bypass a matching usable skill through a browser, Computer Use, direct HTTP, another MCP server, or a local script.",
  "Do not call `request_plugin_install` or open another integration's authorization before this catalog check. When the selected catalog includes a matching enabled and usable skill, use it. Fallback is allowed only when the exact catalog has no relevant skill, its required company or personal connection is not configured or is currently unusable, including when the skill runtime explicitly reports `no_access` or `needs_reconnect`, the operation is unsupported, or the user explicitly chooses non-Trelio; state that exact reason. A missing relevant Trelio skill leaves compatible personal skills and connectors available. Confirmed skill absence or unusability is not a reason to refuse requested work: use an otherwise allowed independent external source or implementation when it is necessary to complete the request. This is fallback, not bypass; it must never reach the same protected system through another route or weaken access controls. Unavailable `list_agent_skills` / `get_agent_skill` or a transient network failure does not by itself establish `no_access`.",
  "Native Trelio MCP and bundled Agent Workspace operations are the primary workspace workflow: do not seek a missing catalog skill merely for task discovery, workspace/Run/context, checkpoint, submit, or restore. The fallback-reason rule applies only when replacing an implementation covered by a relevant skill. This gate does not weaken secret, personal-session, approval, or confirmation boundaries.",
].join("\n\n");
