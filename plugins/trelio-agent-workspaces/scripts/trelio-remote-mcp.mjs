...
export const AGENT_SKILL_ROUTING_INSTRUCTIONS = [
  ...
  "If the current skill response contains `runtimeExecution`, run only its exact command. A leading `trelio-workspace` token is the logical launcher of the currently loaded plugin: resolve it without executing it first; when it is absent from PATH, replace only that token with Node.js 22+ and this same plugin's bundled `scripts/trelio-workspace.mjs`, preserving every remaining token exactly. Never scan plugin caches or select another installed version. This launcher resolution is part of the exact command, not a fallback or local-script bypass; do not announce the normally missing PATH entry or deliberately run a failing probe, and report a launcher problem only if neither approved form can run. If the response contains `remoteMcpExecution`, use only the declared local `trelio-remote-skills` host tools with the returned identity and release. Never bypass a matching skill through a browser, Computer Use, direct HTTP, another MCP server, or a local script.",
  ...
  "Native Trelio MCP control-plane and Agent Workspace operations through Trelio MCP tools and the bundled `trelio-workspace` bridge are the primary workspace workflow, not a fallback from the Agent Skill catalog. Keep the required catalog check for the resolved context, but do not search for or announce a missing catalog skill merely to discover tasks, manage a workspace or Run, read workspace context, checkpoint, submit, or restore. The explicit fallback-reason requirement applies only when choosing another implementation for an operation that a relevant catalog skill could handle.",
