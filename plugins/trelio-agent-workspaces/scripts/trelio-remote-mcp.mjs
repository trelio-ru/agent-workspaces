...
export const AGENT_SKILL_ROUTING_INSTRUCTIONS = [
  ...
  "Do not call `request_plugin_install` or open another integration's authorization before this catalog check. When the selected catalog includes a matching enabled skill, use it; this applies explicitly to a Trelio Google Calendar skill versus the native/recommended Google Calendar plugin. Fallback is allowed only when the exact catalog has no relevant skill, its required connection is not configured, the operation is unsupported, or the user explicitly chooses non-Trelio; state that exact reason. Unavailable `list_agent_skills` / `get_agent_skill` means the skill control plane is unavailable, not that the integration is absent.",
